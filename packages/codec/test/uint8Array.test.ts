...
import { expectErr, expectErrWithSubstring, expectOk } from './assertions';
...
describe('Uint8Array codec', () => {
  describe('basic Uint8Array encoding/decoding', () => {
    ...
    it('should roundtrip correctly', () => {
      ...
      const parsed = expectOk(parse(stringify(json)));
      const decoded = expectOk(uint8Array.jsonCodec.deserialise(parsed));
  ...
  describe('error cases', () => {
    it('should fail on odd length hex string', () => {
      ...
      expectErr(uint8Array.jsonCodec.deserialise(json));
    });

    it('should fail on invalid hex characters', () => {
      const json = "abcg" as Json;
      expectErrWithSubstring(uint8Array.jsonCodec.deserialise(json), 'Invalid character');
    });
  ...
  describe('Sha256 tagged type', () => {
    it('should accept valid 32-byte array', () => {
      ...
      const parsed = expectOk(parse(`"${json}"`));
      const result = expectOk(sha256Codec.deserialise(parsed));
    ...
    it('should reject array with less than 32 bytes', () => {
      ...
      const parsed = expectOk(parse(`"${json}"`));
      expectErrWithSubstring(sha256Codec.deserialise(parsed), "Sha256");
    });
    ...
    it('should roundtrip valid Sha256', () => {
      ...
      const parsed = expectOk(parse(`"${json}"`));
      const decoded = expectOk(sha256Codec.deserialise(parsed));
