...
describe('Integer Codecs', () => {
  describe('Int codec', () => {
    ...
    it('should roundtrip correctly', () => {
      const original = unwrapOk(smallish.number2IntCodec.deserialise(12345));
      const encoded = smallish.json2IntCodec.serialise(original);
      const decoded = unwrapOk(smallish.json2IntCodec.deserialise(encoded));
  ...
  describe('PositiveInt codec', () => {
    ...
    it('should roundtrip correctly', () => {
      ...
      const decoded = unwrapOk(smallish.json2PositiveIntCodec.deserialise(encoded));
  ...
  describe('PositiveBigInt codec', () => {
    ...
    it('should roundtrip correctly', () => {
      const original = unwrapOk(big.bigInt2PositiveBigIntCodec.deserialise(123456789n));
      const encoded = big.json2PositiveBigIntCodec.serialise(original);
      const decoded = unwrapOk(big.json2PositiveBigIntCodec.deserialise(encoded));
  ...
  describe('NonNegativeBigInt codec', () => {
    ...
    it('should roundtrip correctly', () => {
      const original = unwrapOk(big.bigInt2NonNegativeBigIntCodec.deserialise(0n));
      const encoded = big.json2NonNegativeBigIntCodec.serialise(original);
      const decoded = unwrapOk(big.json2NonNegativeBigIntCodec.deserialise(encoded));
      expect(decoded).toBe(0n);
    });
