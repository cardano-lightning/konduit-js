...
export function parse(uri: string, urnScheme?: string): DecodeResult<{ invoiceString: string, decoded: DecodedInvoice}> {
  ...
  return res.andThen((parsedUri: PaymentRequest<string>) => {
    if(parsedUri.options.lightning) {
      return bolt11.parse(parsedUri.options.lightning).match(
        (invoice) => {
          let invoiceInfo: { invoiceString: string, decoded: DecodedInvoice } = { invoiceString: parsedUri.options.lightning as string, decoded: invoice };
          return ok({
            address: parsedUri.address,
            options: { ...parsedUri.options, lightning: invoiceInfo }
          });
        },
        (parseErr) => {
