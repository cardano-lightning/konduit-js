...
export const parse = (val: string) : Result<DecodedInvoice, InvoiceError> => {
  try {
    return bip21.parse(val)
      .andThen(({ options }): Result<DecodedInvoice, InvoiceError> => {
        if (!options.lightning) {
          return err({ message: "No invoice found in BIP21 string" });
        }
        return ok(options.lightning.decoded);
      })
      .orElse((): Result<DecodedInvoice, InvoiceError> => {
        // Try to parse as raw invoice
        return bolt11.parseURI(val).match(
          ({ invoiceString: _str, decoded }) => ok(decoded),
          (error) => err(error as InvoiceError)
        );
      })
      .orElse((_err: bip21.ParseError | { message: string }): Result<DecodedInvoice, InvoiceError> => {
